...
powershell -ExecutionPolicy Bypass -File scripts\gate-phase11.ps1
powershell -ExecutionPolicy Bypass -File scripts\check-package-readiness.ps1
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Error "phase12 gate must run inside the Susun git repository"
        exit $LASTEXITCODE
    }
